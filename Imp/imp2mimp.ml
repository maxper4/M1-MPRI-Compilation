...
let mk_mul e1 e2 = match e1, e2 with
  ...
  | Cst 1, e | e, Cst 1 ->
     e
   | Cst 0, Var x | Var x, Cst 0 ->
      Cst 0
   | Cst y, e | e, Cst y when y mod 2 = 0 ->
      Unop(Sll (y / 2), e)
   | Cst n1, Unop(Muli n2, e) | Unop(Muli n2, e), Cst n1 ->
     Unop(Muli(n1*n2), e)
  | Cst n, e | e, Cst n ->
     Unop(Muli n, e)
  | Unop(Muli n1, e1), Unop(Muli n2, e2) ->
     Unop(Muli(n1*n2), Binop(Mul, e1, e2))
  | _ ->
     Binop(Mul, e1, e2)

let mk_lt e1 e2 = match e1, e2 with
  | Cst n1, Cst n2 -> 
     Cst(if n1 < n2 then 1 else 0)
  | Var v1, Var v2 when v1 = v2 ->
     Cst 0
  | _ ->
     Binop(Lt, e1, e2)

(*let mk_if c t e = match c with
  | Cst 0 -> e
  | Cst _ -> t
  | _ -> If(c, t, e)

let mk_while c b = match c with
  | Cst 0 -> Skip
  | Cst _ -> b
  | _ -> While(c, b)
*)
...
let rec tr_instr = function
  ...
  (*| Imp.If(e, s1, s2) -> mk_if (tr_expr e) (tr_seq s1) (tr_seq s2)
  | Imp.While(e, s) -> mk_while (tr_expr e) (tr_seq s)*)
